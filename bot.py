...
def get_mistakes():
    ...
    ortho_counter = 0   # orthography mistakes
    sentence_start = 0  # a sentence starting with a small letter
    whitespace = 0      # space before a comma/fullstop/etc.
    capital_names = 0   # proper name starting with a small letter
    talk = 0            # colloquialisms
    other = 0           # other types of mistakes
...
def par_info(parameter, mistake_type, filename=filename + '_fixed.csv'):
    content = dict()
    with open(filename, 'r') as f:
            reader = csv.DictReader(f, delimiter=',')
            for ele in reader:
...
Paste a link to a vk group to get started
(and use /startover every time you want to go through a new group)''')
